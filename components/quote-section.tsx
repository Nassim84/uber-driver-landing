...
export function QuoteSection() {
	const { loading, error, submitted, handleSubmit, reset } = useDevisForm();
			...
			<div className="mx-auto grid max-w-7xl items-center gap-12 px-4 sm:px-6 lg:grid-cols-2 lg:gap-16 lg:px-8">
				...
				<div>
					...
					<p className="mt-5 max-w-lg text-lg leading-relaxed text-muted-foreground text-pretty">
						Zakaria, votre chauffeur VTC expérimenté, vous accompagne dans tous
						vos déplacements en Provence-Alpes-Côte d'Azur et ses alentours.
						Réservez simplement, profitez d'un service premium, d'un véhicule
						propre et confortable, et d'une ponctualité sans faille.
					</p>
				...
				<div className="relative">
					...
					<div className="rounded-2xl border border-border bg-card p-6 shadow-2xl sm:p-8">
						<h3 className="font-display text-xl font-bold">
							Demander un devis pour votre{" "}
							<span className="text-primary">trajet</span>
						</h3>
						...
						{submitted ? (
							<div className="mt-8 flex flex-col items-center gap-4 rounded-xl border border-primary/25 bg-primary/8 py-10 text-center">
								...
								<p className="max-w-xs text-sm text-muted-foreground">
									Merci, nous revenons vers vous très rapidement pour confirmer
									votre trajet.
								</p>
								<button
									type="button"
							...
							<form onSubmit={onSubmit} className="mt-6 space-y-4">
								...
								<div className="grid gap-4 sm:grid-cols-2">
									...
									<Field
										...
										name="arrivee"
										placeholder="Aéroport Marseille…"
										required
									/>
								</div>
								<Field
								...
								<button
									type="submit"
									disabled={loading}
									className="w-full rounded-full bg-primary py-3.5 text-sm font-semibold text-primary-foreground shadow-lg shadow-primary/20 transition-transform hover:-translate-y-0.5 disabled:opacity-60 disabled:hover:translate-y-0"
								>
